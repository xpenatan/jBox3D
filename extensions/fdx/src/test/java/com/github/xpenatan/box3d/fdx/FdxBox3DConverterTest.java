...
public class FdxBox3DConverterTest {
    private static final float EPSILON = 0.00001f;

    @Test
    public void convertsVectorsBothWaysIntoReusableOutputs() {
        FakeB3Vec3 box3dSource = new FakeB3Vec3(1.25f, -2.5f, 3.75f);
        Vector3 fdxOut = new Vector3();

        assertSame(fdxOut, FdxBox3DConverter.toFdx(box3dSource, fdxOut));
        assertVector(1.25f, -2.5f, 3.75f, fdxOut);

        FakeB3Vec3 box3dOut = new FakeB3Vec3();
        assertSame(box3dOut, FdxBox3DConverter.toBox3D(new Vector3(-4.0f, 5.0f, 6.0f), box3dOut));
    ...
    public void convertsQuaternionsBothWaysIntoReusableOutputs() {
        ...
        Quaternion fdxOut = new Quaternion();

        assertSame(fdxOut, FdxBox3DConverter.toFdx(box3dSource, fdxOut));
        assertQuaternion(0.1f, 0.2f, 0.3f, 0.9f, fdxOut);

        FakeB3Quat box3dOut = new FakeB3Quat();
        assertSame(box3dOut,
                FdxBox3DConverter.toBox3D(new Quaternion(-0.2f, 0.4f, -0.1f, 0.85f), box3dOut));
    ...
    public void convertsRigidTransformsWithoutAllocatingOutputs() {
        ...
        assertSame(box3dOut, FdxBox3DConverter.toBox3D(position, rotation, box3dOut));
