package com.github.xpenatan.box3d.gdx.wgpu;
...
public class GdxWgpuDebugRenderer extends B3DebugDrawEm {
    ...
    private final WgModelBatch shadowBatch;
    private final WgShapeRenderer lineRenderer;
    private final Environment environment;
    private final WgDirectionalShadowLight shadowLight;
    private final boolean ownsWgModelBatch;
    ...
    private final Array<LineCommand> lineCommands = new Array<LineCommand>(false, 256);
    ...
    public GdxWgpuDebugRenderer() {
        this(new WgModelBatch(), new WgShapeRenderer(), true, true);
    }

    public GdxWgpuDebugRenderer(WgModelBatch modelBatch) {
        this(modelBatch, new WgShapeRenderer(), false, true);
    }

    public GdxWgpuDebugRenderer(WgShapeRenderer lineRenderer) {
        this(new WgModelBatch(), lineRenderer, true, false);
    }

    public GdxWgpuDebugRenderer(WgModelBatch modelBatch, WgShapeRenderer lineRenderer) {
        this(modelBatch, lineRenderer, false, false);
    }

    private GdxWgpuDebugRenderer(WgModelBatch modelBatch, WgShapeRenderer lineRenderer, boolean ownsWgModelBatch,
            boolean ownsLineRenderer) {
        if(modelBatch == null) {
            throw new GdxRuntimeException("WgModelBatch cannot be null");
        }
        if(lineRenderer == null) {
            throw new GdxRuntimeException("WgShapeRenderer cannot be null");
        }
        this.modelBatch = modelBatch;
        this.shadowBatch = new WgModelBatch(new WgDepthShaderProvider());
        this.lineRenderer = lineRenderer;
        this.ownsWgModelBatch = ownsWgModelBatch;
        this.ownsLineRenderer = ownsLineRenderer;
        this.environment = new Environment();
        this.shadowLight = new WgDirectionalShadowLight(2048, 2048, 90.0f, 90.0f, 0.5f, 300.0f);
    ...
    public void render(B3World world, Camera camera) {
        ...
        if(lineCommands.size > 0) {
            ...
            lineRenderer.begin(WgShapeRenderer.ShapeType.Line);
            for(int i = 0; i < lineCommands.size; i++) {
                lineCommands.get(i).draw(lineRenderer);
            }
            lineRenderer.end();
        }
    }
    ...
    private void renderShadows(Camera camera) {
        ...
        try {
            shadowBatch.begin(shadowLight.getCamera(), null, true, RenderPassType.DEPTH_ONLY);
            batchBegan = true;
    ...
    private DebugModel buildModel(B3DebugShape shape) {
        ...
        WgModelBuilder primitiveBuilder = new WgModelBuilder();
    ...
    private PrimitiveInstance buildCapsulePrimitive(WgModelBuilder builder, B3Capsule capsule) {
    ...
    private void line(float x1, float y1, float z1, float x2, float y2, float z2, long color, float alpha) {
        lineCommands.add(new LineCommand(x1, y1, z1, x2, y2, z2, color, alpha));
    }
    ...
    private void toMatrix(Matrix4 out, B3Transform transform) {
        ...
        tempTranslation.set(position.GetX(), position.GetY(), position.GetZ());
        tempRotation.set(qv.GetX(), qv.GetY(), qv.GetZ(), rotation.GetS());
        out.set(tempTranslation, tempRotation, tempScale);
    }
    ...
    private static Color toColor(long hexColor, float alpha) {
    ...
    private static final class LineCommand {
        final float x1;
        final float y1;
        final float z1;
        final float x2;
        ...
        LineCommand(float x1, float y1, float z1, float x2, float y2, float z2, long hexColor, float alpha) {
            this.x1 = x1;
            this.y1 = y1;
            this.z1 = z1;
            this.x2 = x2;
            this.y2 = y2;
            this.z2 = z2;
            this.color = toColor(hexColor, alpha);
        }

        void draw(WgShapeRenderer renderer) {
