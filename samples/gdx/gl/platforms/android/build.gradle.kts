    ...
    doLast {
        ...
        if(missing.isNotEmpty()) {
            throw GradleException(
                "Missing libGDX Android native libraries for ABI(s): ${missing.joinToString()}"
            )
        }
    }
}
...
tasks.register("box3d_gdx_android_jni_build") {
    group = "samples"
    description = "Builds the jBox3D libGDX Android JNI sample."
    dependsOn("assembleDebug")
}

tasks.register<Exec>("box3d_gdx_android_jni_run") {
    group = "samples"
    description = "Installs and launches the jBox3D libGDX Android JNI sample."
    ...
    val command = mutableListOf(
        ...
        "start",
        "-n",
        "com.github.xpenatan.box3d.sample.gdx.android/.Box3DGdxAndroidActivity"
    )
