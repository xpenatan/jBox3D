...
final class JengaStackSample extends AbstractBox3DSample {
    JengaStackSample() {
        ...
        B3Hull block = B3Hull.CreateBox(1.0f, 0.1f, 0.1f);
        B3Quat straight = rotationY(0.0f);
        B3Quat crossed = rotationY(0.5f * (float)Math.PI);

        for(int i = 0; i < 30; i++) {
            B3Quat rotation = (i & 1) == 1 ? straight : crossed;
            float x = (i & 1) == 0 ? 0.8f : 0.0f;
            float z = (i & 1) == 0 ? 0.0f : 0.8f;
            float y = (2.1f * i + 0.5f) * 0.1f;
            addHull(block, B3.DynamicBody(), x, y, z, rotation, 1000.0f, 0.6f, 0.0f, 0.05f);
            addHull(block, B3.DynamicBody(), -x, y, -z, rotation, 1000.0f, 0.6f, 0.0f, 0.05f);
        }
