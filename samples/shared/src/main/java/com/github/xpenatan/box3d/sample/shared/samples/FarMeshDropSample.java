...
/** Exact CreateMeshDrop scene from shared/stability.c at the World/Far Mesh Drop origin. */
final class FarMeshDropSample extends AbstractBox3DSample {
    private static final float OFFSET = 1_000_000.0f;
    private B3Mesh mesh;

    FarMeshDropSample() {
        B3Body ground = createBody(B3.StaticBody(), OFFSET, 0.0f, OFFSET, null);
        ...
        for(int i = 0; i < 20; i++) {
            for(int j = 0; j < 20; j++) {
                ...
                position.Set(OFFSET + 0.5f * (i - 10.0f), 5.0f, OFFSET + 0.5f * (j - 10.0f));
